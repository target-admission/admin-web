...
import { useEmployeeSessionSignOut } from "@/queries/sessions";
...
const SessionCard: React.FC<ISession> = ({
	...
	const { mutateAsync: employeeSessionSignOut } = useEmployeeSessionSignOut();
	...
	const onSubmit = async (id: any) => {
		...
		const res = await handleResponse(() => employeeSessionSignOut(id), [200]);
		message.destroy();
		if (res.status) {
